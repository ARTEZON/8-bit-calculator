...
class Panel:
    def __init__(self):
        ...
        self.info_string = 'Диапазон значений в беззнаковом режиме: от 0 до 255'
...
class Settings:
    ...
    def change_mode(self):
        if self.mode == 0:
            self.mode = 1
            main_window.btn_mode.setText("Режим: со знаком")
            panel.info_string = 'Диапазон значений в знаковом режиме: от -128 до 127'
            main_window.info_label.setText(panel.info_string)

        elif self.mode == 1:
            self.mode = 0
            main_window.btn_mode.setText("Режим: без знака")
            panel.info_string = 'Диапазон значений в беззнаковом режиме: от 0 до 255'
            main_window.info_label.setText(panel.info_string)
...
def update_state(trigger):
    ...
    if (settings.mode == 0 and not(0 <= panel.input_A_decimal + panel.input_B_decimal <= 255)) or (settings.mode == 1 and not(-128 <= panel.input_A_decimal + panel.input_B_decimal <= 127)):
        main_window.info_label.setText('ОШИБКА: Целочисленное переполнение.')
